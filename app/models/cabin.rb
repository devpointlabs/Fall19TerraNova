class Cabin < ApplicationRecord
  ...
  def self.avail_cabins_model(params)
    ...
    Cabin.all.each do |c|          
      ...
      cabins_info << {cabin_id: c.id, cabin_details: {unavailable_dates: takenArray, available_dates: availableDates, price_total: price_total, numberofNights: grandWantedDates.length, aveNightlyRate: price_total/grandWantedDates.length  }}
    end
    return cabins_info
  end
  ...
  def self.single_cabin_avail(params)
    ...
    return {cabin_id: params[:id], cabin_details: {unavailable_dates: takenArray, available_dates: availableDates, price_total: price_total, numberofNights: grandWantedDates.length, aveNightlyRate: price_total/grandWantedDates.length  }}
  end
